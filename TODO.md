...
- [x] **Sync incremental (2026-09-08, mismo día)**: el jefe sigue agregando contenido a su Artifact — se volvió a exportar el CSV unas horas después (148 filas) y hacía falta traer solo lo nuevo sin duplicar lo ya importado. `scripts/import-csv.mjs` ahora es idempotente por defecto (compara contra lo ya cargado en la marca antes de insertar — ver Decisión 14). Trajo 6 piezas nuevas: 3 piezas de email del 07/09 que habían desaparecido de la base entre la primera importación y esta (probablemente borradas sin querer navegando la app — no identificado el momento exacto) y 3 piezas nuevas de "Libros recomendados" (09/09, Facebook/LinkedIn/YouTube). Sofia Contreras quedó en 148 piezas, coincidiendo exactamente con el total del CSV.
...
- [x] **Sync incremental (2026-09-11)**: nuevo export del Artifact del jefe (159 filas, hasta el 09/09). 147 filas ya existían (salteadas por idempotencia), 12 piezas nuevas insertadas — todas del 07 al 09/09 (Historias/Canal de difusión sin ángulo relevante, y el carrusel/reel final de cierre de inscripciones de Gana Tu Año, IG/LinkedIn/Facebook). Sofia Contreras quedó en 160 piezas, coincidiendo exactamente con el total del CSV. CSV local borrado tras confirmar el conteo (no se versiona, sigue en `.gitignore`).
...
- [x] **(2026-09-08, a pedido de la usuaria) "Todas las marcas" pasa a ser el default de entrada**: sin `?brand=` en la URL (entrar directo a `/`) ya no cae en Mastery Haus, cae en la vista combinada — `page.tsx` trata `brandSlug === undefined` igual que `ALL_BRANDS_SLUG`. Cualquier slug real en la URL (`?brand=mastery-haus`, `?brand=sofia-contreras`) sigue aislando esa marca sola, sin cambios. Verificado en navegador real (usuario de prueba creado/borrado por API): tras login, el selector queda en "Todas las marcas" sin tocar nada, con los contadores combinados. `npm run build` limpio.
