...
## 2026-09-08 — Decisión 15: SOP ("Cómo funciona el Tablero") como página interna, no como Artifact externo

**Contexto**: la usuaria pidió un SOP simple del Tablero — para quién es, qué es, quién lo hizo, qué hace hoy, cómo usarlo paso a paso y cuidados básicos — pensado para que lo entienda cualquiera del equipo sin conocimiento técnico, incluyendo al jefe (que todavía no migró del mockup a esta app).

**Primer intento**: se armó como Artifact de Claude (HTML publicado con link compartible), replicando los tokens de diseño de `mockup.html` (mismos colores/radios/sombras/tipografía) para que se sintiera parte del mismo producto. La usuaria lo compartió con el equipo, pero después preguntó si iba a quedar reflejado "del lado del proyecto" (con un link tipo footer) para poder verlo desde ahí.

**Decisión**: pasar el contenido a una página propia dentro de la app, `src/app/ayuda/page.tsx`, en vez de mantenerlo como Artifact externo.
- **Por qué**: la usuaria pidió explícitamente que "se vaya actualizando conforme agreguemos o quitemos funcionalidad" — un Artifact aparte vive fuera del repo y depende de que alguien se acuerde de republicarlo a mano cada vez que cambia algo en la app; una página dentro del código se edita en el mismo lugar y momento que el resto del proyecto.
- **Protección**: `/ayuda` no está en la lista de excepciones de `src/proxy.ts` (`/login`, `/api/*`), así que queda protegida por el login automáticamente, igual que el resto de la app — no hizo falta tocar el proxy.
- **Evitar que se desincronice**: el glosario de estados y la lista de plataformas soportadas se arman leyendo `STATUS_META`/`PLATFORM_META` de `src/lib/pieces.ts` (la misma fuente de verdad que usa el resto de la UI) en vez de tener las etiquetas escritas a mano — si un estado cambia de nombre/color, o se agrega una plataforma, esta página lo refleja sin tocarla. El resto del contenido (qué hace hoy, pasos, cuidados) sigue siendo texto a mano, igual que `TODO.md`/`DECISIONS.md` — se edita cuando cambie algo relevante.
- **Descubrimiento real al escribir el contenido**: el estado de pieza `error` se muestra en la UI como **"Demorado"** (`STATUS_META.error.label`, ver `PieceItem.tsx` línea 196: "Esta pieza está marcada Demorado — revisá qué falta para destrabarla") — es contenido trabado al que le falta algo para avanzar, **no** un error de guardado del Tablero. El Artifact original (primer intento) lo explicaba mal, conflando ese estado con el indicador de guardado (`save-status`) del header, que es un concepto distinto. Se corrigió el texto en ambos lados (la página nueva y el Artifact, que se dejó igual de actualizado a pedido de la usuaria).
- **El Artifact no se borró**: sigue existiendo (ya compartido con el equipo) pero ahora incluye un link a la página interna como "la versión que se actualiza sola"; la fuente de verdad pasa a ser `/ayuda`.
